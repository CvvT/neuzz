...
def process_data():
    ...
    # get MAX_FILE_SIZE
    cwd = os.getcwd()
    # max_file_name = call(['ls','-S', cwd+'/neuzz_in/']).split('\n')[0].rstrip('\n')
    # MAX_FILE_SIZE = os.path.getsize(cwd+'/neuzz_in/'+max_file_name)
    MAX_FILE_SIZE = 512
    ...
    np.savetxt("fit_bitmap.out", fit_bitmap, fmt="%d")
...
# index of edge, two seeds, model, layer, enumerator
def gen_adv2(f,fl,model,layer_list,idxx,splice):
    adv_list = []
    loss = layer_list[-2][1].output[:,f]
    grads = K.gradients(loss, model.input)[0] # gradient of loss with respect to input
...
def gen_mutate2(model, edge_num, sign):
    ...
    if len(new_seed_list) < edge_num:
        # random choose edge_num values from range(len)
